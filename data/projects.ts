...
export const projects: Project[] = [
  // ===== B2B SaaS / Enterprise =====
  {
    slug: "chagok",
    name: "CHAGOK",
    summary: "이혼 소송 AI 파라리걸 & 멀티모달 증거 파이프라인",
    description:
      "이미지/음성/영상/PDF/카카오톡 텍스트 등 다양한 형식의 증거를 분석하는 Parser Registry 패턴을 구현했습니다. 케이스별 격리 RAG 인덱스(case_rag_{case_id})로 증거 교차 오염을 방지합니다.",
    intent: "법률 서비스는 오류가 용납되지 않습니다. 67개 테스트 케이스를 먼저 작성하고, SHA-256 해싱으로 증거 무결성(Chain of Custody)을 보장하며, Human-in-the-loop(변호사 수동 승인 필수) 구조를 채택했습니다.",
    category: "b2b-saas",
    stack: ["Next.js", "TypeScript", "FastAPI", "PostgreSQL", "AWS", "Qdrant", "OpenAI GPT-4o"],
    techStackMap: {
      Frontend: ["Next.js 14", "TypeScript", "Tailwind CSS", "Zustand"],
      Backend: ["FastAPI", "Python 3.11", "Pydantic"],
      Database: ["PostgreSQL (RDS)", "DynamoDB (Metadata)", "Qdrant (Vector DB)"],
      Infra: ["AWS S3", "AWS Lambda (AI Worker)", "ECS Fargate", "CloudFront"],
      AI: ["OpenAI GPT-4o Vision", "Whisper (STT + 화자 분리)", "OCR"],
    },
    highlights: [
      "Parser Registry 패턴: Image→GPT-4o Vision / Audio→Whisper+화자분리 / Video→음성추출→Whisper / PDF→텍스트+OCR / Text→KakaoTalk 형식 감지",
      "케이스 격리 RAG: 각 케이스별 독립 인덱스(case_rag_{case_id})로 증거 교차 오염 방지",
      "이중 RAG 아키텍처: 법률 지식 RAG + 사건별 증거 RAG 분리",
      "SHA-256 증거 무결성: 모든 증거 파일 해싱, Chain of Custody 보장",
      "67개 테스트 케이스: pytest 백엔드 테스트로 법률 서비스 신뢰성 확보",
      "3-tier AWS 구조: Frontend(Vercel) + Backend(ECS) + AI Worker(Lambda) 비동기 처리",
    ],
    featured: true,
    metrics: {
      duration: "3개월",
  ...
  },
  {
    slug: "numna-road",
    name: "NumnaRoad",
    summary: "24/7 eSIM 자동 발급 플랫폼 + Circuit Breaker 4사 통합",
    description:
      "n8n 노코드의 한계(웹훅 유실, 타임아웃, 디버깅 어려움)를 극복하고자 직접 코드로 마이그레이션했습니다. 4개 eSIM 제공자(RedteaGO/eSIMCard/MobiMatter/Airalo)를 Circuit Breaker 패턴으로 통합했습니다.",
    intent: "핵심 비즈니스 로직을 블랙박스에 의존하면 안 됩니다. n8n에서 Vercel Serverless로 이관하며 상태 기계를 도입하고, 인프라 비용을 $0로 최적화했습니다.",
    category: "b2c-consumer",
    stack: ["Next.js 14", "TypeScript", "PocketBase", "Stripe", "Resend"],
    techStackMap: {
      Frontend: ["Next.js 14 (App Router)", "React Query", "Zustand", "MUI v7"],
      Backend: ["Vercel Serverless Functions", "PocketBase (Go)"],
      Infra: ["Oracle Cloud Free Tier (영구 무료)", "Cloudflare Tunnel"],
      Automation: ["Stripe Webhook", "Resend Email API", "State Machine"],
    },
    highlights: [
      "n8n→Code 마이그레이션: 웹훅 유실/타임아웃 문제 해결, 핵심 로직 직접 제어",
      "Circuit Breaker 패턴: 3회 연속 실패 시 제공자 차단, 우선순위 기반 자동 Failover",
      "4사 eSIM 통합: RedteaGO(100) → eSIMCard(80) → MobiMatter(60) → Airalo(40) → Manual(10)",
      "$0 인프라 비용: Oracle Cloud Free Tier + Cloudflare Tunnel + PocketBase",
      "주문 처리 < 10초: 결제-발급-발송 완전 자동화",
    ],
    featured: true,
    metrics: {
      duration: "6주",
  ...
  },
  {
    slug: "careguide",
    name: "CareGuide CKD",
    summary: "만성콩팥병(CKD) 환자 멀티에이전트 AI 건강관리 플랫폼",
    description:
      "CKD 환자들에게 정확한 의학 정보를 제공하기 위해, 도메인별 특화 에이전트(논문/영양/복지/트렌드)와 하이브리드 검색(Keyword 40% + Semantic 60%)을 결합한 멀티 에이전트 시스템을 개발했습니다.",
    intent: "'논문 찾아줘', '뭐 먹어도 돼?', '복지 혜택 알려줘' 등 다양한 질문에 하나의 프롬프트로 대응하면 정확도가 떨어집니다. AgentManager가 의도를 분류하고 적합한 전문 에이전트로 라우팅하는 구조를 채택했습니다.",
    category: "ai-agent",
    stack: ["Python", "FastAPI", "React", "MongoDB", "OpenAI", "Pinecone", "Cohere"],
    techStackMap: {
      Frontend: ["React", "TypeScript", "Recharts"],
      Backend: ["FastAPI", "Python 3.11"],
      Data: ["MongoDB (문서 + 벡터)", "Pinecone (대규모 벡터 검색)"],
      AI: ["LangGraph (Multi-Agent)", "OpenAI GPT-4o", "Cohere (리랭킹)", "PubMed API"],
    },
    highlights: [
      "4개 전문 에이전트: ResearchPaper/Nutrition/MedicalWelfare/TrendVisualization - 각 도메인별 최적화된 프롬프트",
      "하이브리드 검색: BM25(40%) + Vector Search(60%) + Cohere 리랭킹으로 정확도 극대화",
      "세션 관리: 30분 타임아웃, 세션당 20K 토큰 제한, 에이전트별 컨텍스트 추적",
      "데이터 정규화: 2,224,451개 → 13,102개 (99.4% 감소, 정확도 유지)",
      "MongoDB Vector Search + Pinecone 연동으로 13,000+ 의료 데이터 RAG 구축",
    ],
    featured: true,
    metrics: {
      duration: "8주",
  ...
  },
  {
    slug: "salon-mate",
    name: "SalonMate",
    summary: "뷰티/살롱 AI 마케팅 자동화 + Clean Architecture",
    description:
      "소규모 뷰티샵 사장님들의 리뷰 응대와 SNS 마케팅 시간을 줄이기 위해, Clean Architecture(Controller-Agent-Service 분리)와 JWT Auth(Access 30min, Refresh 7days)를 적용한 AI 마케팅 솔루션을 개발했습니다.",
    intent: "소상공인에게 마케팅은 생존이지만 시간이 부족합니다. Controller-Agent-Service 분리로 유지보수성을 확보하고, JWT 토큰 갱신 전략으로 보안과 사용성을 모두 잡았습니다.",
    category: "b2b-saas",
    stack: ["Next.js", "TypeScript", "FastAPI", "Supabase", "OpenAI", "LangChain"],
    techStackMap: {
      Frontend: ["Next.js 14", "TypeScript", "Tailwind CSS"],
      Backend: ["FastAPI", "Clean Architecture (Controller-Agent-Service)"],
      Database: ["Supabase (PostgreSQL)"],
      Auth: ["JWT Auth (Access 30min, Refresh 7days)"],
      AI: ["OpenAI GPT-4o", "LangChain"],
    },
    highlights: [
      "Clean Architecture: Controller-Agent-Service 분리로 테스트 용이성 확보",
      "JWT Auth 전략: Access Token 30분 + Refresh Token 7일",
      "고객 리뷰 감성 분석 기반의 맞춤형 답글 자동 생성",
      "시술 사진 분석 → 인스타그램 캡션 및 해시태그 자동 생성",
      "마케팅 성과(노출, 예약 전환) 시각화 대시보드",
    ],
    metrics: {
      duration: "6주",
      role: "풀스택 단독",
  ...
  },
  {
    ...
    summary: "AI 실시간 모의면접 & SSE 스트리밍 (E2E < 2.5초)",
    description:
      "음성 입력→텍스트 변환→LLM 응답→음성 출력까지 10초 이상 걸리면 실제 면접 느낌이 나지 않습니다. SSE 기반 파이프라인으로 E2E 2.5초 내 응답을 달성했습니다.",
    intent: "자기소개서를 단순히 500자씩 자르면 '지원동기'와 '성장과정'이 섞여서 RAG 정확도가 떨어집니다. 한국어 섹션 인식 청킹(800자 + 100자 오버랩)으로 문맥을 유지했습니다.",
    category: "b2c-consumer",
    stack: ["Next.js 16", "TypeScript", "Supabase", "OpenAI GPT-4o", "Whisper", "Cohere"],
    techStackMap: {
      Frontend: ["Next.js 16", "TypeScript", "SSE Client"],
      Backend: ["Next.js API Routes", "SSE Streaming"],
      Database: ["Supabase (PostgreSQL)", "Qdrant (Vector DB)"],
      AI: ["OpenAI GPT-4o", "Whisper (STT)", "TTS", "Cohere (리랭킹)"],
    },
    highlights: [
      "SSE 스트리밍 파이프라인: STT(500-800ms) → LLM 스트리밍(800-1200ms) → TTS 청크(300-500ms)",
      "한국어 섹션 인식 청킹: 지원동기/성장과정/경력 등 7개 섹션 자동 감지, 800자 청킹 + 100자 오버랩",
      "하이브리드 검색: Vector + BM25 + Cohere 리랭킹, 정확도 >70%",
      "3가지 면접관 페르소나(채용담당자/HR/시니어) + MBTI 기반 맞춤형 질문",
      "8축 역량 평가 모델 + 발화 습관(WPM, 필러워드) 분석 리포트",
    ],
  ...
  },
  {
    slug: "racelab",
    name: "KRace (레이스랩)",
    summary: "한국 공영경주(경마/경륜/경정) 통합 정보 플랫폼 + 13팩터 예측 엔진",
    description:
      "파편화된 한국 공영경주(경마/경륜/경정) 정보를 통합하고 실시간성을 확보하기 위해, 통합 데이터 파이프라인과 13개 팩터 기반 예측 모델을 갖춘 정보 플랫폼을 개발했습니다.",
    intent: "공영경주 데이터의 파편화와 API 불안정성(1초~30초 응답 시간, 간헐적 장애)을 해결하고, 단순 통계를 넘어선 정량적 예측 모델로 사용자에게 실질적인 가치를 제공하고자 했습니다.",
    category: "b2c-consumer",
    stack: ["Next.js 14", "TypeScript", "Drizzle ORM", "Redis", "Vercel Cron", "PostgreSQL"],
    techStackMap: {
      Frontend: ["Next.js 14", "React", "TypeScript", "Storybook"],
      Backend: ["Next.js API Routes", "Drizzle ORM", "Vercel Cron"],
      Database: ["PostgreSQL", "Redis (캐싱)"],
      Infra: ["Vercel", "Bull Queue"],
    },
    highlights: [
      "KSPO 및 각 시행체 데이터를 통합 수집/정제하는 ETL 파이프라인 구축",
      "13개 팩터(주로상태/거리/레이팅/기수승률 등) 기반 Softmax 예측 엔진 + Kelly Criterion 배팅 비율 계산",
      "Exponential Backoff + Redis 캐싱(5분) + Fallback 더미 데이터로 API 장애 시에도 서비스 연속성 보장",
      "모노리스→마이크로서비스 전환: racelab-core(Private) + racelab-web(Public) 분리",
      "TDD 기반 개발: Jest 유닛/API 테스트 + Playwright E2E 테스트",
    ],
  ...
  },
  {
    ...
    summary: "Astro Islands + SSG 면적 변환 계산기",
    description:
      "Astro의 Islands Architecture를 활용해 빠른 로딩 속도와 React 컴포넌트 하이드레이션을 양립했습니다. client:load vs client:visible을 전략적으로 선택하여 초기 로딩 성능을 최적화했습니다.",
    intent: "Astro에서 React 컴포넌트 하이드레이션 전략(client:load vs client:visible)을 상황에 맞게 적용하는 것이 핵심이었습니다. SSG로 빌드하여 Cloudflare Pages에서 빠르게 서빙합니다.",
    category: "b2c-consumer",
    stack: ["Astro 4", "React", "TypeScript", "Tailwind CSS", "Cloudflare Pages"],
    techStackMap: {
      Framework: ["Astro 4 (Islands Architecture)", "React"],
      Build: ["SSG (Static Site Generation)", "Vite"],
      Testing: ["Vitest", "Playwright E2E"],
      Deploy: ["Cloudflare Pages"],
    },
    highlights: [
      "Astro Islands: client:load(즉시 하이드레이션) vs client:visible(뷰포트 진입 시) 전략적 선택",
      "SSG 빌드: 정적 사이트 생성으로 빠른 초기 로딩",
      "Material 3 디자인 시스템: 일관되고 직관적인 UX",
      "SpaceVisualizer: 실제 가구/물건 크기와 비교 가능한 공간 시각화",
      "Vitest 유닛 테스트 + Playwright E2E 테스트",
    ],
    metrics: {
      duration: "1주",
      role: "풀스택 단독",
  ...
  },
  {
    slug: "some-some",
    name: "Some Some (썸썸)",
    summary: "60fps Flutter 커플 터치 게임 + Sin/Cos 8자 경로",
    description:
      "두 사람이 동시에 터치하는 게임에서 프레임 드랍이 발생하면 몰입감이 깨집니다. CustomPainter + Ticker로 60fps를 보장하고, Sin/Cos 조합으로 8자 경로를 생성했습니다.",
    intent: "Flutter 기본 애니메이션 대신 저수준 제어(CustomPainter + Ticker)를 선택했습니다. elasticOut 이징으로 게임 피드백에 탄성 느낌을 주고, Haptic Feedback 패턴(heavyImpact → lightImpact → vibrate)으로 촉각 경험을 설계했습니다.",
    category: "b2c-consumer",
    stack: ["Flutter 3.x", "Dart 3.3.0+", "CustomPainter", "Ticker"],
    techStackMap: {
      Framework: ["Flutter 3.x", "Dart 3.3.0+"],
      Rendering: ["CustomPainter (저수준 렌더링)", "Ticker (60fps 보장)"],
      Animation: ["elasticOut 이징", "Sin/Cos 8자 경로 생성"],
      UX: ["Haptic Feedback 패턴", "Material Design 3"],
    },
    highlights: [
      "60fps 보장: CustomPainter + Ticker로 Flutter 기본 애니메이션 대신 저수준 제어",
      "Sin/Cos 8자 경로: sin(t) * cos(t) 조합으로 부드러운 곡선 경로 생성",
      "Haptic Feedback 패턴: heavyImpact → lightImpact → vibrate 순차 진동으로 촉각 경험 설계",
      "elasticOut 이징: 게임 피드백에 탄성 느낌 적용",
      "두 사용자 터치 경로 동기화 + 물리적 접촉 유도 레벨 디자인",
    ],
    metrics: {
      duration: "2주",
      role: "모바일 단독",
  ...
  },
  {
    slug: "talk-studio",
    name: "TalkStudio",
    summary: "Spec-First 대화 스크린샷 생성 앱 (React 19)",
    description:
      "카카오톡, 텔레그램, 인스타그램, 디스코드 스타일 대화 스크린샷을 생성하는 SPA입니다. Spec-First 개발 워크플로우(/pm→/architect→/dev→/qa)를 적용했습니다.",
    intent: "명세서 → 설계 → 구현 → 테스트 순서로 개발하되, 보안 테스트(XSS, 입력 검증)를 자동화 필수 단계로 포함했습니다.",
    category: "developer-tools",
    stack: ["React 19", "Zustand 5", "Tailwind CSS 4", "Vite 7"],
    techStackMap: {
      Frontend: ["React 19", "TypeScript", "Zustand 5"],
      Build: ["Vite 7", "Tailwind CSS 4"],
      Testing: ["Vitest", "XSS/입력 검증 자동화"],
    },
    highlights: [
      "Spec-First 워크플로우: /pm(명세서) → /architect(설계) → /dev(구현) → /qa(테스트)",
      "3-Column 레이아웃: Sidebar(80px) - Editor(flex-1) - Preview(flex-1.2)",
      "Zustand 상태 통합: Theme, StatusBar, Profiles, Messages",
      "보안 테스트 필수: XSS 방지, 입력 검증 자동화",
      "4개 플랫폼 스타일 지원: KakaoTalk, Telegram, Instagram DM, Discord",
    ],
    metrics: {
      duration: "3주",
      role: "풀스택 단독",
  ...
  },
  {
    ...
    summary: "GLP-1 AI 식단 코칭 + 3가지 AI 페르소나",
    description:
      "GLP-1 다이어트 약물 사용자의 식단 관리를 위해, 3가지 AI 코치 페르소나(냥이/댕댕이/꿀꿀이)와 11,086개 한국 음식 DB를 결합한 음성 식사 로깅 챗봇을 개발했습니다.",
    intent: "자연스러운 음성 식사 로깅 경험을 제공하기 위해 친근한 동물 페르소나를 도입했습니다. 식약처 공식 문서 RAG로 정확한 약물 정보를 보장합니다.",
    category: "ai-agent",
    stack: ["React", "Vite", "FastAPI", "Supabase", "OpenAI GPT-4o-mini", "LlamaIndex"],
    techStackMap: {
      Frontend: ["React", "Vite", "Tailwind CSS"],
      Backend: ["FastAPI", "Python"],
      Database: ["Supabase (PostgreSQL)", "11,086개 음식 DB"],
      AI: ["OpenAI GPT-4o-mini", "LlamaIndex RAG"],
    },
    highlights: [
      "3가지 AI 코치 페르소나: 냥이(고양이)/댕댕이(개)/꿀꿀이(돼지) - 사용자 성향별 맞춤",
      "11,086개 한국 음식 DB: 자연어 식단 기록 및 칼로리 자동 분석",
      "식약처 공식 문서 RAG: 정확한 GLP-1 약물 정보 및 Q&A",
      "음성 식사 로깅: 자연스러운 대화형 식단 기록",
    ],
    metrics: {
      duration: "1주",
      role: "풀스택 단독",
  ...
  },
  {
    ...
    summary: "Zero-Trust 직장 내 괴롭힘 신고 플랫폼 + AES-256-GCM 암호화",
    description:
      "익명 인시던트 보고 + 파트너 대시보드 + 암호화 증거 체인을 제공합니다. AES-256-GCM + Key Versioning으로 Payload를 암호화하고, HMAC으로 Evidence Chain 변조를 감지합니다.",
    intent: "보안은 나중이 아닙니다. 처음부터 AES-256, SHA-256, HMAC을 설계에 포함했습니다. Public Code는 평문 저장 금지, SHA-256으로만 조회하며, PII Guard가 민감 정보를 자동 감지/마스킹합니다.",
    category: "b2b-saas",
    stack: ["Next.js 15", "Neon PostgreSQL", "Drizzle ORM", "AES-256-GCM"],
    techStackMap: {
      Frontend: ["Next.js 15", "TypeScript", "Tailwind CSS"],
      Backend: ["Next.js API Routes", "Drizzle ORM"],
      Database: ["Neon PostgreSQL (Serverless)", "Qdrant (Vector Search)"],
      Security: ["AES-256-GCM + Key Versioning", "HMAC Evidence Chain", "SHA-256 Hash"],
      AI: ["GPT-4 (PII 탐지)", "LangChain", "Embedding Models"],
      Compliance: ["PIPA (개인정보보호법)", "GDPR-ready Architecture"],
    },
    highlights: [
      "AES-256-GCM 암호화: Payload 암호화 + Key Versioning으로 키 순환 지원",
  ...
  // ===== CTO-Focused Projects (GitHub Analysis) =====
  {
    slug: "ansim-golf",
    name: "Ansim Golf",
    summary: "골프장 자동화 플랫폼 + Gemini 1.5 Flash AI 파싱",
    description:
      "KakaoTalk/Excel 비정형 데이터를 Gemini 1.5 Flash로 자동 파싱하고, Cloudflare D1(Edge SQLite)로 50ms 이하 응답을 달성한 골프장 운영 자동화 플랫폼입니다.",
    intent: "골프장 관리 시스템이 실패하는 이유는 기술적 한계가 아니라 운영자(종종 50-60대)가 복잡한 UI를 탐색할 수 없기 때문입니다. 시니어 친화적 UI(큰 터치 타겟, 높은 대비)를 우선시했습니다.",
    category: "b2b-saas",
    stack: ["Next.js", "TypeScript", "Cloudflare D1", "Cloudflare Workers", "Gemini 1.5 Flash"],
    techStackMap: {
      Frontend: ["Next.js 14", "React", "Tailwind CSS"],
      Backend: ["Cloudflare Workers (Edge)", "Cloudflare D1 (Edge SQLite)"],
      AI: ["Gemini 1.5 Flash (비정형 텍스트 구조화)"],
      Payment: ["Toss Payments (에스크로)"],
      UX: ["Senior-Friendly UI", "Large Touch Targets", "High Contrast"],
    },
    highlights: [
      "Gemini 1.5 Flash AI 파싱: KakaoTalk/Excel 비정형 텍스트 → 구조화된 예약 데이터",
      "Edge-first 아키텍처: Cloudflare D1(SQLite) + Workers로 50ms 이하 응답",
      "시니어 친화적 UI: 큰 터치 타겟, 단순화된 워크플로우, 높은 대비",
      "자동 정산 시스템: 수작업 정산 80% 감소",
      "노쇼 자동 청구: SMS/알림톡 통합",
      "Toss Payments 에스크로: 안전한 거래 보장",
    ],
    featured: true,
    metrics: {
      duration: "6주",
